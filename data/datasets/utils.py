...
def get_iteration_path(root_dir, fix_iter = -1):
    if fix_iter != -1:
        return os.path.join(root_dir,'checkpoint_%d.pt' % fix_iter)

    if not os.path.exists(root_dir):
        return None
    file_names = glob.glob(os.path.join(root_dir,'checkpoint_*.pt'))
    max_iter = -1
    for file_name in file_names:
        ...
        if temp > max_iter:
            max_iter = temp
    if not os.path.exists(os.path.join(root_dir,'checkpoint_%d.pt' % max_iter)):
        return None
    return os.path.join(root_dir,'checkpoint_%d.pt' % max_iter)
...
def get_iteration_path_and_iter(root_dir, fix_iter = -1):
    ...
    if not os.path.exists(os.path.join(root_dir,'checkpoint_%d.pt' % max_iter)):
        return None
    return os.path.join(root_dir,'checkpoint_%d.pt' % max_iter), max_iter